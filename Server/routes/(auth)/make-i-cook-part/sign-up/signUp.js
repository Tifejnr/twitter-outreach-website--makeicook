...
const signUpMakeICookRouter = express.Router();

signUpMakeICookRouter.post("/", async (req, res) => {
  //get request sent
  const bodyRequest = await req.body;
  const { fromExtension } = bodyRequest;
  ...
  if (fromExtension) {
    ...
    let accountUser = await userMakeICook.findOne({ email: bodyRequest.email });

    if (accountUser)
      return res.json({ alreadyRegistered: "User already registered" });

    accountUser = new userMakeICook(
      _.pick(bodyRequest, ["name", "email", "password"])
    );
...
export default signUpMakeICookRouter;
